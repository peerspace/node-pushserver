...
app.post('/subscribe', function (req, res) {
    if (req.body.subscriptionId) {
        ...
        oneSignal.subscribeCallbackFunc(req.body, (err, result) => {
            if (err) {
...
app.post('/unsubscribe', function (req, res) {
    ...
    if (data.user) {
        ...
        oneSignal.unsubscribeCallbackFunc({ssoId: data.ssoId, subscriptionId: data.subscriptionId}, (err, result) => {
            if (err) {
    ...
    PushAssociations.findSubscriptionsForUser(req.body.users[0], (err, subscriptions) => {
        if(err) {
            ...
            if(subscriptions.length > 0) {
                ...
                oneSignal.sendCallbackFunc(notifs[0], (err, result) => {
                    if(err){
