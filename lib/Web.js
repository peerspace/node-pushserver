...
app.post('/subscribe', function (req, res) {
    if (req.body.subscriptionId) {
        console.log('Redirecting to push-notifications-2 POST /subscribe: ', JSON.stringify(req.body))
        oneSignal.postRequestCallbackFunc('subscribe', req.body, (err, result) => {
            if (err) {
                console.log('Error during subscription creation: ', err.message);
                res.send(err.message)
            } else {
                console.log('Subcription is stored: ', req.body.subscriptionId);;
                res.send('subscribed')
            }      
        });
    } else {
...
app.post('/unsubscribe', function (req, res) {
    ...
    if (data.user) {
        ...
        console.log('Redirecting to push-notifications-2 POST /unsubscribe: ', JSON.stringify(data))
        oneSignal.postRequestCallbackFunc('unsubscribe', {ssoId: data.ssoId, subscriptionId: data.subscriptionId}, (err, result) => {
            if (err) {
                console.log('Error during subscription removing: ', err.message);
                res.send(err.message);
            } else {
                console.log('Subcription is removed: ', req.body.subscriptionId);;
                res.send('unsubscribed - oneSignal');
            }    
        });
    }
    else {
...
app.post('/send', function (req, res) {
    ...
    /**
     * users array always has only one user included
     * we don't send transactional pushes two multiple users in one request
     */
    const ssoId = req.body.users[0];
    //the request always contains only one notification
    const notification = notifs[0];
    PushAssociations.findSubscriptionsForUser(ssoId, (err, subscriptions) => {
        if (err) {
            console.log(`Error while reading subscriptions from DB for ${ssoId}`, err);
            res.status(400).send(err.message);
        } else {
            if (subscriptions.length > 0) {
                console.log(`Subscriptions are found. The notification should be sent via OneSignal for ${ssoId}`);
                
                oneSignal.postRequestCallbackFunc('send', notification, (err, result) => {
                    if (err) {
                        console.log(`Error while sending push notifications for ${ssoId}`)
                        res.status(400).send(err.message);
                    } else {
                        console.log('SENT VIA ONESIGNAL ', req.body.users, result);
                        res.status(200).send();
                    }
                });
            } else {
                console.log('Subscriptions are NOT found. Sending via APN');
                var notificationsValid = sendNotifications(notifs);
                console.log('SENT VIA OLD METHOD ', req.body.users, notificationsValid);
...
function sendNotifications(notifs) {
    // check if notification payload is present and is valid for each notification
    ...
    notifs.forEach(function (notif) {
        ...
        var fetchUsers = users ? pushAssociations.getForUsers : pushAssociations.getAll,
            callback = function (err, pushAssociations) {
                ...
                push.send(pushAssociations, notif);
            },
