...
app.post('/subscribe', function (req, res) {
    if (req.body.subscriptionId) {
        oneSignal.subscribeCallbackFunc(req.body, () => {
            res.send('subscribed via OneSignal')
        });
    } else {
...
app.post('/unsubscribe', function (req, res) {
    ...
    //DO WE NEED TO HANDLE USERS UNSUBSCRIBE??
    if (data.user) {
        ...
        console.log('Redirecting to push-notifications-2: ', {ssoId: data.ssoId, subscriptionId: data.subscriptionId});
        oneSignal.unsubscribeCallbackFunc({ssoId: data.ssoId, subscriptionId: data.subscriptionId}, () => {
            res.send('unsubscribed in OneSignal')
        });
    }
    else {
