...
app.post('/send', function (req, res) {
    ...
    PushAssociations.findSubscriptionsForUser(req.body.users[0], (err, subscriptions) => {
        if (err) {
            console.log('Error while reading subscriptions from DB: ',err);
            res.status(400).send();
        } else {
            if (subscriptions.length > 0) {
                console.log('Subscriptions are found. Sending via OneSignal');
                oneSignal.postRequestCallbackFunc('send', notifs[0], (err, result) => {
                    if (err) {
