...
app.post('/send', function (req, res) {
     ...
     * we don't send transactional pushes to multiple users in one request
     */
    ...
    PushAssociations.findSubscriptionsForUser(ssoId, (err, subscriptions) => {
        if (err) {
            ...
            if (subscriptions.length > 0) {
                ...
                oneSignal.postRequestCallbackFunc('send', req.body, (err, result) => {
                    if (err) {
                ...
                console.log(`Subscriptions are NOT found for ${ssoId}. Sending via APN`);
                var notificationsValid = sendNotifications(notifs);
                console.log('Sent directly via APN ', req.body.users, notificationsValid);
