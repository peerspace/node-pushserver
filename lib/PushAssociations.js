...
var initialize = _.once(function () {
    var db = mongoose.connect(config.get('mongodbUrl'), {useNewUrlParser: true});
    mongoose.connection.on('error', errorHandler);

    var pushAssociationSchema = new Schema({
        user: {
            type: 'String',
            required: true
        },
    ...
    PushAssociation = mongoose.model('PushAssociation', pushAssociationSchema);
    Subscriptions = mongoose.model('Subscriptions', SubscriptionsSchema);

    return module.exports;
});

const findSubscriptionsForUser = function (ssoId, callback) {
    PushAssociation.find({sso_id: ssoId}, (err, subscriptions) => {
        if(err){
            console.log('Error while retrieving subscriptions for sso_id: ',ssoId);
            callback(err);
        }else{
...
var removeForUser = function (user, callback) {
...
var removeDevice = function (token, callback) {
